...
class IslandSpecialization(nn.Module):
    ...
    def update(
        ...
        # Per-island per-class accuracy ratio
        with torch.no_grad():
            ...
            # Smooth accuracy = (accurate + epsilon) / (exposure + epsilon)
            eps = 0.01
            smooth_acc = (accurate_sum + eps) / (exposure_sum + eps)  # (num_active, num_classes)

            # EMA update: scores = alpha * smooth_acc + (1-alpha) * scores
